...
$('#createAdmin').addEventListener('click',async()=>{const password=$('#authPassword').value;if(password.length<8){authMessage('请先输入至少 8 位密码。','error');return}const form=$('#authForm');setBusy(form,true);authMessage('正在创建管理员账号…');const {data,error}=await db.auth.signUp({email:ADMIN_EMAIL,password});setBusy(form,false);if(error){authMessage(error.message,'error');return}if(data.session){currentUser=data.user;renderAuth();$('#authDialog').close();notice('管理员账号已创建并登录。')}else authMessage('账号已创建，请到邮箱完成确认后再登录。','success')});
