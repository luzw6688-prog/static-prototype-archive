...
function renderCounts(){
  ...
  const directions=new Set(state.projects.map(p=>p.category).filter(x=>x&&x!=='未分类')).size;
  $('#categoryDirections').textContent=String(directions).padStart(2,'0');
  ...
  $('#experimentProjects').textContent=String(state.projects.filter(p=>p.stage===2||p.stage===3).length).padStart(2,'0');
  $('#scaleCandidates').textContent=String(state.projects.filter(p=>p.stage===4).length).padStart(2,'0');
...
function filtered(){return state.projects.filter(p=>{const text=`${p.name} ${p.category} ${p.subcategory} ${p.trend} ${p.owner}`.toLowerCase();return (selectedStage==='all'||p.stage===Number(selectedStage))&&(!selectedTrend||p.trend===selectedTrend)&&text.includes(searchTerm.toLowerCase())})}
