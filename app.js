...
let state={projects:[],trends:[],relations:[],activity:[],gateDefinitions:[]},deletedProjects=[],currentUser=null,isAdmin=false;
...
async function loadData({quiet=false}={}){
  ...
  const [projectsRes,trendsRes,relationsRes,metricsRes,tasksRes,activityRes,gateDefinitionsRes,gatesRes,historyRes,experimentsRes]=await Promise.all([
  ...
  const failure=[projectsRes,trendsRes,relationsRes,metricsRes,tasksRes,activityRes,gateDefinitionsRes,gatesRes,historyRes,experimentsRes].find(r=>r.error);
...
function riskProjectIds(cutoff=today()){return new Set(state.projects.filter(p=>p.tasks.some(t=>!t.done&&t.due<cutoff)||trendsFor(p.id).some(t=>/(风险|阻塞|异常|逾期)/.test(t.status||''))).map(p=>p.id))}
...
function filtered(){const stats=decisionStats();return state.projects.filter(p=>{const related=trendsFor(p.id),text=`${p.name} ${p.category} ${p.subcategory} ${p.owner} ${related.map(t=>t.name).join(' ')}`.toLowerCase(),commandMatch=!selectedCommandFilter||(selectedCommandFilter==='decision'?stats.pendingIds:stats.riskIds).has(p.id),trendMatch=!selectedTrend||related.some(t=>t.id===selectedTrend);return commandMatch&&(selectedStage==='all'||p.stage===Number(selectedStage))&&trendMatch&&text.includes(searchTerm.toLowerCase())})}
...
function renderProjects(){
  ...
  const selectedTrendRecord=state.trends.find(t=>t.id===selectedTrend),clues=[];if(selectedCommandFilter)clues.push(selectedCommandFilter==='decision'?'「待决策项目」':'「风险与逾期」');if(selectedStage!=='all')clues.push(`「${stages[selectedStage]}」阶段`);if(selectedTrendRecord)clues.push(`趋势「${selectedTrendRecord.name}」`);if(searchTerm)clues.push(`搜索「${searchTerm}」`);
