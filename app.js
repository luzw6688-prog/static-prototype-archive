...
async function loadData({quiet=false}={}){
  ...
  const [projectsRes,trendsRes,metricsRes,tasksRes,activityRes,gateDefinitionsRes,gatesRes,historyRes]=await Promise.all([
  ...
  const failure=[projectsRes,trendsRes,metricsRes,tasksRes,activityRes,gateDefinitionsRes,gatesRes,historyRes].find(r=>r.error);
...
function gateChecklist(p){const status=gateStatus(p);return `<div class="gate-summary"><div><span>${esc(stages[p.stage])}完成度</span><b>${status.done} / ${status.total}</b></div><div class="gate-progress"><i style="width:${status.total?Math.round(status.done/status.total*100):0}%"></i></div></div><div class="gate-list">${status.definitions.map(g=>`<label class="gate-item ${status.completed.has(g.gate_key)?'completed':''}"><input type="checkbox" data-gate="${esc(g.gate_key)}" data-stage="${g.stage}" data-project="${p.id}" ${status.completed.has(g.gate_key)?'checked':''} ${isAdmin?'':'disabled'}><span><b>${esc(g.label)}</b><small>${g.required?'必填门槛':'建议门槛'}</small></span></label>`).join('')}</div>${status.missing.length?`<div class="gate-blocker"><b>暂不能推进，还缺少 ${status.missing.length} 项</b><ul>${status.missing.map(g=>`<li>${esc(g.label)}</li>`).join('')}</ul></div>`:'<div class="gate-ready"><b>当前阶段必填门槛已完成</b><span>可以申请推进至相邻的下一阶段。</span></div>'}`}
...
function renderStageDialogState(){const p=project(pendingProject);if(!p)return;const target=pendingStageMode==='admin'?Number($('#stageTarget').value):pendingStage,delta=target-p.stage,forward=delta>0,missing=forward?gateStatus(p).missing:[],reasonRequired=delta<0||Math.abs(delta)>1;pendingStage=target;$('#advanceTitle').textContent=`${delta<0?'回退':delta>1?'跨阶段调整':'推进'}至「${stages[target]}」` ;$('#advanceCopy').textContent=`「${p.name}」将从「${stages[p.stage]}」调整至「${stages[target]}」。本次变更会写入不可变更的阶段历史。`;$('#stageReasonLabel').textContent=reasonRequired?'调整原因（必填）':'推进说明（选填）';$('#stageReason').required=reasonRequired;$('#stageReasonHint').textContent=reasonRequired?'回退或跨阶段调整必须说明依据与后续处理。':'不填写时，将记录为“当前阶段必填门槛全部完成”。';$('#stageGateSummary').innerHTML=missing.length?`<div class="stage-dialog-blocked"><b>无法进入后续阶段，还缺少：</b><ul>${missing.map(g=>`<li>${esc(g.label)}</li>`).join('')}</ul></div>`:'<div class="stage-dialog-ready">阶段规则检查通过</div>';$('#stageSubmit').disabled=Boolean(missing.length);$('#stageSubmit').textContent=missing.length?'门槛未完成':delta<0?'确认回退 →':delta>1?'确认跨阶段调整 →':'确认推进 →'}
